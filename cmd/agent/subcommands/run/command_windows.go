...
func StartAgentWithDefaults(ctxChan <-chan context.Context) (<-chan error, error) {
	...
	go func() {
		err := fxutil.OneShot(func(
			...
			err := startAgent(&cliParams{GlobalParams: &command.GlobalParams{}}, log, flare, telemetry, sysprobeconfig, server, capture, serverDebug, rcclient, logsAgent, forwarder, sharedSerializer, otelcollector, hostMetadata, invAgent)
			if err != nil {
				return err
			}
...
func run(log log.Component,
	...
	return commonRun(log, config, flare, telemetry, sysprobeconfig, server, capture, serverDebug, forwarder, rcclient, metadataRunner, demux, sharedSerializer, cliParams, logsAgent, otelcollector, hostMetadata, invAgent)
}
