...
// Package util offers helpers and building block to easily generate payload for the inventory product.
//
...
//		cp := &customePayload{}
//		cp.Init(deps.Config, deps.Log, deps.Serializer, cp.getPayload, "custome.json")
//		return provides{
//			Comp:          cp,
...
//	}
//
// func (cp *customePayload) getPayload() marshaler.JSONMarshaler { ... }
package util
...
// Init initialize InventoryPayload. 'getPayload' will be called each time a new payload needs to be generated. The
// function returns a pre-configured flare.Provider and metetadata `runner.provider`
func (i *InventoryPayload) Init(conf config.Component, l log.Component, s serializer.MetricSerializer, getPayload PayloadGetter, flareFileName string) {
	i.Enabled = configutils.InventoryEnabled(conf)

	i.MinInterval = time.Duration(conf.GetInt("inventories_min_interval")) * time.Second
	if i.MinInterval <= 0 {
		i.MinInterval = defaultMinInterval
	}

	i.MaxInterval = time.Duration(conf.GetInt("inventories_max_interval")) * time.Second
	if i.MaxInterval <= 0 {
		i.MaxInterval = defaultMaxInterval
	}

	i.conf = conf
	i.log = l
	i.serializer = s
	i.getPayload = getPayload
	i.flareFileName = flareFileName
...
func (i *InventoryPayload) Collect(ctx context.Context) time.Duration {
	i.m.Lock()
	defer i.m.Unlock()
