...
func (s *HTTPTestSuite) TestUnknownMethodRegression() {
	...
	srv, srvDoneFn := testutil.HTTPServer(t, "1.1.1.1:0", testutil.Options{
		EnableTLS:       false,
		EnableKeepAlive: true,
	})
	srvPort := testutil.GetPort(t, srv.Addr)
	...
	requestFn := requestGenerator(t, testutil.SwitchIP(t, srv.Addr, "2.2.2.2"), emptyBody)
	for i := 0; i < 100; i++ {
		requestFn()
	}
	...
	require.Equal(t, 100, requestsSum)
}
